...
g = rcosdesign(BETA,SPAN,fs/BR, "sqrt");
g = g/sum(g);
yup = filter(g,1,xup);
...
% 1) Tomo la salida del ecualizador a tasa de símbolo y elimino ceros
ak_hat_v = out_eq_down(out_eq_down ~= 0);
ak_hat_v = ak_hat_v(:);

% 2) Ajusto longitudes para que data_v y ref_v tengan el mismo largo
Ltx = length(x);
Lrx = length(ak_hat_v);
Lmin = min(Ltx, Lrx);

ref_v  = x(1:Lmin);
data_v = ak_hat_v(1:Lmin);

% 3) Llamo al BER checker
M = 4;        % QPSK
guard = 'auto';   % o por ejemplo 2000 si querés fijar la guarda a mano

[ber, n_errors] = my_ber_checker(data_v, ref_v, M, guard);

fprintf('BER (EbNo = %.1f dB) = %e  (errores = %d)\n', ...
        EbNo, ber, n_errors);
